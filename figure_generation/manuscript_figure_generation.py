...
panel_one = (
    sg.fromfile(f"{str(corpora_comparison_path)}/svg_files/corpora_kl_divergence.svg")
)
...
panel_two = (
    sg.fromfile(f"{str(corpora_comparison_path)}/svg_files/biorxiv_pmc_frequency_odds.svg")
)
...
panel_three = (
    sg.fromfile(f"{str(corpora_comparison_path)}/svg_files/biorxiv_pmc_frequency_bar.svg")
)
...
panel_four = (
    sg.fromfile(f"{str(corpora_comparison_path)}/svg_files/preprint_published_frequency_odds.svg")
)
...
panel_four.moveto(30, 820, scale=1)
...
panel_five = (
    sg.fromfile(f"{str(corpora_comparison_path)}/svg_files/preprint_published_frequency_bar.svg")
...
panel_five.moveto(620, 800, scale=1)
...
panel_two_label = sg.TextElement(10, 400, "B", size=22, weight="bold")
panel_three_label = sg.TextElement(610, 400, "C", size=22, weight="bold")
panel_four_label = sg.TextElement(30, 810, "D", size=22, weight="bold")
panel_five_label = sg.TextElement(620, 810, "E", size=22, weight="bold")


# In[9]:


figure_one = sg.SVGFigure("1400", "1200")
figure_one.append([
    panel_one, 
    panel_two, 
    panel_three,
...
panel_one = (
    sg.fromfile(f"{str(word_association_path)}/pca_01_cossim_word_cloud.svg")
)

panel_one_size = (
    float(panel_one.root.attrib['width']),
    float(panel_one.root.attrib['height'])
)
...
panel_two = (
    sg.fromfile(f"{str(word_association_path)}/pca_02_cossim_word_cloud.svg")
)
...
panel_three = (
    sg.fromfile(f"{str(pca_association_path)}/scatterplot_files/pca01_v_pca02.svg")
)
...
panel_four = (
    sg.fromfile(f"{str(pca_association_path)}/category_box_plot/category_box_plot_pc1.svg")
)
...
panel_one_label = sg.TextElement(10, 20, "A", size=22, weight="bold")
panel_one_image_label = sg.TextElement(200, 385, "PCA 1", size=22, weight="bold")
...
panel_five_label = sg.TextElement(10, 800, "E", size=22, weight="bold")


# In[18]:


figure_two = sg.SVGFigure("1600", "1200")
figure_two.append([
    panel_one,
    panel_two,
    panel_three,
...
panel_one = (
    sg.fromfile(f"{str(article_distance_path)}/biorxiv_article_distance.svg")
)
...
panel_two = (
    sg.fromfile(f"{str(article_distance_path)}/distance_bin_accuracy.svg")
)
...
panel_three = (
    sg.fromfile(f"{str(article_distance_path)}/publication_rate.svg")
)
...
panel_one = (
    sg.fromfile(f"{str(publication_delay_path)}/article_distance_vs_publication_time.svg")
)
...
panel_two = (
    sg.fromfile(f"{str(publication_delay_path)}/version_count_vs_publication_time.svg")
)
...
panel_three = (
    sg.fromfile(f"{str(time_to_publication_path)}/preprint_category_halflife.svg")
)
